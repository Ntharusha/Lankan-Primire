...
Welcome to the **Lankan Primire** repository! 🎬 This is a full-stack movie digital ticketing application. 

This README primarily outlines the **DevOps architecture, CI/CD pipeline, and infrastructural design** behind the project. It provides an overview of how we achieve a seamless, automated flow from development to production.
...
- **Observability**: Prometheus & Grafana (Local/WIP)
...
Our pipeline uses **GitHub Actions** (`.github/workflows/ci.yml`) to govern code quality. When code is pushed or a PR is opened against the `main` branch, the following automated jobs are triggered:
...
Both the client and server applications are containerized using **Docker**, ensuring consistency across development, testing, and production environments.
...
- **AWS Provider**: Automatically targets an AWS region.
- **VPC & Security**: Provisions a tight Security Group restricting to essential ports: SSH (22 - Whitelisted IP only), HTTP (80 - Client), and specific App Ports (5000/3000).
...
If the CI phase passes successfully, GitHub Actions proceeds to the **Deploy phase**, orchestrating the application update directly on AWS EC2 without any downtime:
...
- Leverages Docker run configurations with proper environment variable seeding (e.g., `MONGODB_URI`, `JWT_SECRET`, port bindings).
- Connects the containers on the custom `lankan-net` private bridge network so they securely interact.
...
We utilize `docker-compose` to replicate our production architecture locally. This guarantees that what passes locally will work on the cloud!
...
- Finalize the full Kubernetes transition migrating off simple `docker run` commands.
- Stand up Grafana and hook it smoothly into Prometheus for realtime observability triggers and scaling warnings.
