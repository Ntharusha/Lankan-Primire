  ...
  return (
    <div className="relative min-h-[100vh] w-full flex items-center justify-start p-8 md:p-32 bg-black overflow-hidden">
      ...
      <div
        ...
        style={{
          backgroundImage: `linear-gradient(to right, rgba(0,0,0,0.9) 20%, rgba(0,0,0,0.4) 50%, transparent 100%), linear-gradient(to top, rgba(0,0,0,0.8) 10%, transparent 40%), url(${getBackdropUrl(movie.backdrop_path)})`
        }}
      ></div>
      ...
      <div className="relative z-20 max-w-2xl text-white ml-4 md:ml-12">
        ...
        <div className="flex flex-col gap-4 mb-8">
          <div className="flex items-center space-x-4 text-sm font-semibold tracking-wide uppercase text-red-500">
            {movie.genres && movie.genres.map((g, i) => (
              <React.Fragment key={g._id || i}>
                <span>{g.name}</span>
                {i < movie.genres.length - 1 && <span className="w-1 h-1 bg-gray-500 rounded-full"></span>}
              </React.Fragment>
            ))}
          </div>
          ...
          <div className="flex items-center space-x-3">
            ...
            <div className="flex items-center text-yellow-500 text-sm font-bold">
              <Star className="w-4 h-4 mr-1 fill-current" /> {movie.vote_average}
            </div>
          </div>
        </div>
        ...
        <div className="flex flex-wrap gap-4">
          <Link to={`/movies/${movie._id}`} className="group flex items-center bg-red-600 hover:bg-red-700 transition-all duration-300 px-8 py-4 rounded-xl text-base font-bold shadow-lg shadow-red-600/20 text-white">
            Book Now
          ...
          <button className="flex items-center bg-white/5 hover:bg-white/10 border border-white/20 transition-all px-8 py-4 rounded-xl text-base font-bold backdrop-blur-sm text-white">
            Watch Trailer
          </button>
        </div>
      </div>
